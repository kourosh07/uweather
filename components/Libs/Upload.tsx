...
export type UploadStatuses = Array<{
  name: string;
  size: number;
  percent: number;
  url: string;
...
export default (props: {
  ...
  function removeCropted() {
    ...
    statuses = statuses.filter(
      (st) => names.includes(st.name) || st.percent == 100
    );
  }
    ...
    if (fileinput.files && fileinput.files[0]) {
      for (var f of Array.from(fileinput.files)) {
        statuses.push({
          ...
          size: f.size,
          percent: 0,
          url: `https://cdn.ituring.ir/qeupload/${
            global.user.uid
          }/${f.name.toLowerCase()}`,
        });
      }
    ...
    if (fileinput.files && fileinput.files[0]) {
      for (var f of Array.from(fileinput.files)) {
        if (props.extensionfilter && props.extensionfilter.length > 0) {
          if (
            !props.extensionfilter.find((ext) =>
        ...
        onChange={(e) => {
          for (let f of e.target.files) {
            if (f.size > (props.maxsize || 12 * 1024 * 1024)) {
              alerter(
                lang.uploaderrsize.replace(
                  "XX",
                  Math.ceil(
                    (props.maxsize || 12 * 1024 * 1024) / (1024 * 1024)
                  ) + " MB"
                )
              );
              e.target.value = null;
  ...
  return (
    <div
      style={{ display: "flex", marginTop: 5, marginLeft: props.marginLeft }}
    >
        ...
        if (st.percent == 100) {
          if (
            ...
            return (
              <div
                key={key}
                style={{ position: "relative", lineHeight: 0.7, marginTop: -5 }}
              >
                <a
                  style={{
                    position: "relative",
                ...
                <div
                  ...
                  onClick={() => {
                    statuses = statuses.filter(
                      (it) => !(it.name.toLowerCase() == st.name.toLowerCase())
                    );
                    props.on?.(statuses);
                ...
                >
                  <img
                    ...
                    src={global.cdn("/files/close.svg")}
                    alt={props.alt}
                  />
                </div>
              </div>
            ...
            if (f.size > (props.maxsize || 12 * 1024 * 1024)) {
              alerter(
                lang.uploaderrsize.replace(
                  "XX",
                  Math.ceil(
                    (props.maxsize || 12 * 1024 * 1024) / 1024
                  ).toLocaleString(global.lang.region) + " kb"
                )
              );
              e.target.value = null;
              return;
